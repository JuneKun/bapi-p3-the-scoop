...
function updateComment(url, request){
   ...
   //console.log(url)
   //console.log(request)
   //console.log(!id || !requestComment || !requestComment.body);
   //console.log('------------')
   ...
   if(!id || !requestComment || !requestComment.body){
     ...
     response.status = 200;
     response.body = {body: savedComment};
   }
...
function createComment(url, request) {
  ...
  //console.log(requestComment.articleId)
  //console.log(database.articles[requestComment.articleId])

  if (requestComment && //if requestComment exists
      requestComment.body && //if body exists
      requestComment.username && //if username exists
      database.articles[requestComment.articleId] && //if article ID exists in database
      database.users[requestComment.username] //if username exists in database
      ){
          const comment = {
...
function deleteComment(url, request){
  ...
  if(savedComment && database.users[savedComment.username]){
    const dbUserComments = database.users[savedComment.username].commentIds;
    const dbArticleComments = database.articles[savedComment.articleId].commentIds;
    const dbUser = database.users[savedComment.username];
    const dbArticle = database.articles[savedComment.articleId];

    database.comments[id] = null;
    const dbUserCommentIdIndex = dbUserComments.indexOf(id);
    dbUserComments.splice(dbUserCommentIdIndex,1);
    const dbArticleCommentIdIndex = dbArticleComments.indexOf(id);
    dbArticleComments.splice(dbArticleCommentIdIndex, 1);
    response.status = 204;
  }else{
    response.status = 404;
  };
...
function loadDatabase(){
  ...
  try{
    ...
    const indentedJson = JSON.stringify(database, null, 4);
